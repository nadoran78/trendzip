...
- 현재 집중 영역: 첫 운영 숏폼 검수본의 운영 등록과 사람 승인

## 한 줄 상태

백엔드의 트렌드 수집·저장·조회 API와 주요 화면이 공개됐고 Vercel Analytics와 GA4/GTM 운영 검증을 완료했다. 반복 콘텐츠 방지 정책을 적용한 운영 초안 생성에 이어, 실제 TTS·MP4 렌더링과 최신 아티팩트만 사람이 승인할 수 있는 검수 게이트까지 구현했다.
...
- 구현됨: 새 DRAFT 준비·TTS·렌더를 묶은 생성 스크립트와 사람 확인 뒤 등록·검수를 재개 가능한 형태로 묶은 운영 스크립트
- 운영 배포됨: V8 migration과 렌더 아티팩트·검수 API
- 운영 검증 대기: 첫 렌더 아티팩트 등록, 사용자의 MP4 전체 재생과 승인·수정 요청·반려 결정
...
1. 첫 운영 검수본의 운영 등록과 사람 전체 재생 검수
...
- 첫 운영 검수본은 로컬에서 생성·자동 검증했고 운영 DB에도 V8 migration을 배포했다. 렌더 아티팩트 등록과 사람 결정은 아직 실행하지 않았으며, 자동 검사는 사람의 발음·음량·전체 흐름 확인을 대신하지 않는다.
