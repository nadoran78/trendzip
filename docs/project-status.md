...
- 현재 집중 영역: 첫 운영 숏폼 초안 검토와 후속 렌더링·승인 파이프라인 준비

## 한 줄 상태

백엔드의 트렌드 수집·저장·조회 API와 주요 화면이 공개됐고 Vercel Analytics와 GA4/GTM 운영 검증을 완료했다. Gemini TTS와 Remotion 동기화 스파이크에 이어, 반복 콘텐츠를 방지하는 숏폼 선정·편집·발행 정책과 후속 자동화 상태를 정의했다.
...
- 미구현: 운영 초안의 TTS·영상 렌더링과 최종 결과 사람 승인 파이프라인
...
1. 첫 운영 `DRAFT`의 TTS·영상 렌더링과 최종 결과 승인·반려·재생성 게이트
