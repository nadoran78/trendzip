...
- 구현 중: 제작 이력 기반 운영 키워드 자동 선정, 편집 계약 보정, 안정성 비교와 형식별 근거 시점 진단을 포함하는 dry-run 파이프라인
...
- 숏폼 샘플은 고정 fixture 기반 실제 TTS까지 검증했고 운영 후보 수집·Gemini 편집 계획·30일 중복 판정·제작 이력 예약과 manifest 생성을 연결했다. 실제 3회 운영 dry-run에서 발견한 참조 ID, 안정성, 세대 표현 문제와 `WHY_NOW` 최근 근거 진단을 보완했으며 운영 데이터 재검증이 남아 있다.
