...
- 현재 집중 영역: 숏폼 TTS 생성과 오디오 동기화 기술 검증

## 한 줄 상태

백엔드의 트렌드 수집·저장·조회 API와 주요 화면이 공개됐고 Vercel Analytics와 GA4/GTM 운영 검증을 완료했다. Gemini TTS 생성과 실제 음성 길이 기반 Remotion 장면 동기화 스파이크는 구현과 수동 품질 검수를 마치고 병합을 기다린다.
