...
- 구현·운영 검증됨: Gemini 근거 선택, 결정적 fact card·문안 조립, 시스템 topicKey·eventKey, 출처 검토 경고와 단계별 실패 진단을 포함하는 manifest·dry-run v3 파이프라인
- 개선 필요: 결정적 composer의 문장 단위 길이 처리, 근거 기반 정보 밀도와 편집 형식별 다양성
...
1. 운영 숏폼 composer의 문장 완결성과 편집 형식별 다양성 개선
...
- 숏폼 샘플은 고정 fixture 기반 실제 TTS까지 검증했고 운영 후보 수집·30일 중복 판정·제작 이력 예약과 manifest 생성을 연결했다. Gemini를 후보·형식·근거 원문 선택기로 제한한 뒤 실제 3회 dry-run이 모두 첫 호출에 성공해 가용성과 식별자 안정성을 확인했다. 다만 긴 발췌의 문장 중간 절단과 제목 재진술 중심의 획일적인 문안이 확인돼 발행 전 composer 개선이 필요하다.
