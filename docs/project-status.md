...
- 구현 중: 제작 이력 기반 운영 키워드 자동 선정, 근거·톤·길이 계약 위반의 1회 보정과 반복 실패를 격리하는 dry-run 파이프라인
...
1. 숏폼 정책을 운영 키워드 후보와 제작 초안에 연결하는 입력 설계
...
- 숏폼 샘플은 고정 fixture 기반 실제 TTS까지 검증했고 운영 후보 수집·Gemini 편집 계획·30일 중복 판정·제작 이력 예약과 manifest 생성을 연결했다. 실제 3회 운영 dry-run에서 후보 밖 근거 영상 ID로 2회 실패하고 성공 표본 1건을 안정적으로 표시하는 문제가 확인되어 참조 ID 보정과 안정성 판정 보완이 남아 있다.
