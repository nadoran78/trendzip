...
- 현재 집중 영역: 제작 이력 기반 운영 숏폼 후보 자동 선정과 초안 연결
...
1. 최초 운영 숏폼 `DRAFT` 예약과 제작 이력 기반 중복 판정 검증
...
- 숏폼 샘플은 고정 fixture 기반 실제 TTS까지 검증했고 운영 후보 수집·30일 중복 판정·제작 이력 예약과 manifest 생성을 연결했다. 영상 설명·태그를 포함한 검증 근거로 Brief를 만들고 별도 Gemini가 문안을 작성하는 2단계 구조를 실제 운영 후보로 검증했다.
