...
- 구현 중: 제작 이력 기반 운영 키워드 자동 선정, 구조화 대본 계약 검증과 반복 실패를 격리하는 dry-run 파이프라인
...
- 숏폼 샘플은 고정 fixture 기반 실제 TTS까지 검증했고 운영 후보 수집·Gemini 편집 계획·30일 중복 판정·제작 이력 예약과 manifest 생성을 연결했다. 운영 dry-run 코드 검증은 완료했지만 실제 운영 데이터 비교와 최종 렌더링 승인·게시 파이프라인은 아직 완료하지 않았다.
