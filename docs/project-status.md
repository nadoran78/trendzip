...
- 현재 집중 영역: 운영 키워드 기반 숏폼 파이프라인의 다음 작업 정의

## 한 줄 상태

백엔드의 트렌드 수집·저장·조회 API와 주요 화면이 공개됐고 Vercel Analytics와 GA4/GTM 운영 검증을 완료했다. Gemini TTS 생성과 실제 음성 길이 기반 Remotion 장면 동기화 스파이크까지 완료했으며, 다음 운영 숏폼 작업 정의를 기다린다.
...
1. 운영 키워드 기반 숏폼 대본 생성과 사람 승인 모델 설계
