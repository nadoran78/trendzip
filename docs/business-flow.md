...
→ 명시적 로컬 명령으로 장면별 TTS를 `1.3x`로 동기화한 워터마크 없는 `PUBLIC_CANDIDATE` H.264·AAC MP4·대표 장면 생성
→ 원본·음성·props·영상 hash와 규격 검증 후 렌더 아티팩트 등록
→ 사람 전체 재생 검수 후 최신 아티팩트에 APPROVED·NEEDS_REVISION·REJECTED 결정 기록
→ 승인 전에는 업로드하지 않음
→ 승인된 동일 공개 후보 MP4는 운영자가 YouTube Studio에서 비공개 업로드 후 수동 공개
```
...
TTS와 영상 렌더링은 API 비용과 로컬 자원을 쓰는 명시적 운영 스크립트에서만 실행한다. 생성 스크립트는 등록이나 승인을 수행하지 않고 워터마크 없는 `1.3x` 공개 후보 MP4 하나를 만든다. 검수 스크립트는 사람이 그 동일 영상을 확인해 결정을 입력한 뒤에만 렌더 등록과 결정을 순서대로 처리한다. 검수 상태는 영상 안의 문구가 아니라 render manifest와 운영 API에만 보존한다. 렌더 아티팩트는 파일 hash, 영상 규격 또는 재생 속도가 달라지면 다른 결과로 취급한다. 사람 결정은 등록된 최신 아티팩트에만 허용하고, `NEEDS_REVISION` 뒤 재렌더하면 이전 아티팩트와 검수 이력은 보존한다. 대본이나 근거를 바꿀 때는 기존 콘텐츠 hash를 재사용하지 않고 새 `DRAFT`를 만든다. 초기 운영은 승인 결과를 YouTube Studio에서 비공개 업로드 후 수동 공개하며, 업로드 자동화는 이 흐름을 검증한 뒤 진행한다.
