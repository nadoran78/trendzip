...
- 시작일: 2026-08-21
- 마지막 갱신: 2026-08-23
- 다음 행동: 운영 인증값으로 3회 dry-run을 다시 실행해 참조 ID 보정, 안정성, 세대 표현과 근거 시점 진단 결과를 검토한다.
...
- 통과: 미디어 테스트 83건과 타입 검사
- 통과: 보완된 Gemini 편집 계획 테스트 8건과 미디어 타입 검사
- 통과: `editorial-plan-validation.test.mjs`의 복구 가능 오류, 일반 오류, 미지원 코드 판정 3건
- 통과: 참조 ID 구조화·보정·재실패를 검증하는 편집 계획 테스트 15건
- 통과: 안정성 비교 가능 여부와 보고서 집계를 검증하는 `operational-dry-run.test.mjs` 5건
- 통과: 세대 근거 오류의 거부·보정, 양쪽 관측 세대와 30~40대 설명 대상 허용을 포함한 미디어 집중 테스트 24건
...
- 후보 밖 참조 ID 보정에서는 주제·사건 식별자를 유지하되 오류가 발생한 `relatedKeywordIds` 또는 `evidenceVideoIds`만 오류 상세의 `allowedValues` 안에서 다시 고른다.
- 관심·화제·인기·순위의 주체는 같은 키워드가 실제 후보로 존재하는 `TEEN`, `TWENTY`만 허용한다. 30~40대는 설명을 읽는 대상으로 언급할 수 있지만 관측된 유행 세대로 단정하지 않는다.
