...
```text
앱인토스 WebView 번들
  -> Vercel AppInToss BFF (공개 읽기 전용)
  -> Cloudflare Access Service Token (Vercel 서버 전용)
  -> Cloudflare Tunnel
  -> Spring Boot API
```

기존 `trendzip.nadoran.com` 웹과 앱인토스 앱은 서로 다른 클라이언트로 운영한다. 둘은 같은 트렌드 데이터 계약을 사용하지만, 앱인토스 전용 UI 번들과 BFF 경로를 별도로 둔다.

## 확정 사항

- 구현 방식은 기존 React·웹 경험을 가장 빠르게 재사용할 수 있는 WebView SDK 3.x 기반으로 한다.
- 첫 출시에는 토스 로그인, 결제, 광고, 푸시와 앱인토스 서버 API를 도입하지 않는다.
...
### 2. Vercel AppInToss BFF 추가

- 기존 Next.js에 `/api/appintoss/*` Route Handler를 추가한다.
- BFF가 서버 전용 `API_BASE_URL`, `CLOUDFLARE_ACCESS_CLIENT_ID`, `CLOUDFLARE_ACCESS_CLIENT_SECRET`으로 Spring Boot API를 호출한다.
- 응답 DTO는 앱에 필요한 읽기 모델만 노출하고, 쓰기·운영 API·Swagger 경로는 노출하지 않는다.
- 타임아웃, 오류 응답, 캐시 제어와 요청 단위 rate limit을 적용한다.

### 3. CORS와 보안 경계 적용

`appName` 확정 후 Vercel BFF의 허용 Origin을 아래 두 개로 제한한다.
...
- `Access-Control-Allow-Origin`은 요청 Origin을 검증한 뒤에만 반환한다.
- CORS는 브라우저 정책일 뿐 API 인증 수단이 아니므로, BFF는 공개 데이터만 제공하고 rate limit·입력 검증·응답 크기 제한을 적용한다.
- Cloudflare Access Service Token, Gemini·YouTube·DB 비밀정보, Vercel 환경변수는 앱 번들과 `.ait` 파일에 포함하지 않는다.
...
- 앱인토스 SDK 버전과 Origin 정책이 변경될 때마다 CORS·저장소·BFF 호출을 재검증한다.
...
공식 자료 확인일: 2026-08-29. 콘솔, SDK와 심사 정책은 변경될 수 있으므로 구현과 제출 직전에 다시 확인한다.
