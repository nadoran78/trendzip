...
- 여러 `EXP-*` 학습 단위를 순차적으로 수행하는 장기 실험은 `experiment/<주제>` 브랜치 하나에서 이어갈 수 있다.
- `experiment/*` 브랜치에는 같은 실험 주제의 `EXP-*` 작업만 두고, 각 학습 단위는 독립적인 커밋과 검증 결과로 구분한다.
- 하나의 `experiment/*` 브랜치에서 `IN_PROGRESS` 또는 `BLOCKED`인 작업은 한 번에 하나만 두며, 완료한 학습 단위는 전체 실험의 병합 여부를 결정할 때까지 `REVIEW`로 유지할 수 있다.
...
- 장기 실험의 연속된 `EXP-*` 학습 단위를 제외하고 서로 다른 작업을 한 브랜치에 섞지 않는다.
...
- 코드나 문서를 변경한 세션은 최종 보고 전에 `./dev/review`를 실행한다.
- 커밋을 이미 생성했다면 `./dev/review --commit HEAD`로 해당 커밋의 리뷰 대상을 복구한다.
- 리뷰 대상 파일은 알파벳순이 아니라 의존성과 실행 흐름을 이해하기 좋은 순서로 나열한다.
- 각 파일에는 변경 이유와 사용자가 중점적으로 확인할 부분을 한 줄로 설명한다.
- 테스트는 대응하는 구현 뒤에, 문서는 구현과 검증 뒤에, lock과 자동 생성 파일은 마지막에 배치한다.
- 완료 보고는 구현 결과, 리뷰할 파일 순서, 사용자 실습 항목, 실행한 검증과 결과, 미실행 검증, 커밋 여부와 다음 단계 순서로 작성한다.
- 사용자 실습 항목이 없으면 없다고 명시하고, 커밋하지 않은 경우에도 이를 명시한다.
